...
/*@
type_synonym va = integer

type_synonym cn_chunk_hdr = {
  va header_address, // the VA of the start of the chunk_hdr
  integer alloc_size, // exactly as in the C
  integer mapped_size,  // exactly as in the C
  integer va_size      // implicit in the C: the total va space size of this chunk
...
predicate [nounfold] void Cn_char_array(pointer p, integer size)
{
        assert(0 <= size);
        assert(0 <= (integer)p);
        assert((integer)p <= 18446744073709551615);
        assert((integer)p + size <= 18446744073709551616);
...
predicate void Cn_char_array_with_offset(pointer p, integer size, integer offset)
{
        ...
        assert((integer)p + offset <= 18446744073709551615);
        assert((integer)p + offset + size <= 18446744073709551616);
...
predicate void Conditional_Cn_char_array(pointer p, integer size, boolean cond)
{
        if (cond) {
...
predicate void MaybeCn_char_array(pointer p, integer size)
{
        if (ptr_eq(p, NULL)) {
                return;
        } else {
...
predicate void MaybeCn_char_array_with_offset(pointer p, integer size, integer offset)
{
        if (ptr_eq(p, NULL)) {
                return;
        } else {
...
{
        assert((integer)p > 0);
        assert((integer)p + sizeof<struct hyp_allocator> > (integer)p);
...
predicate ({cn_chunk_hdr Hdr, struct list_head Node}) Cn_chunk_hdr_inner(pointer header_address, cn_hyp_allocator_core ha, option_u64 va_size_opt, boolean check_node, option_u64 alloc_size_opt, boolean valid_mapped_size)
{
        ...
        let va_size = match (va_size_opt) {
                Option_u64_none {} => {
                        (Cn_list_is_last(hdr.node, ha.head) ? end : (integer)my_container_of_chunk_hdr(hdr.node.next) ) - (integer)header_address
                }
        ...
        assert(
                check_node implies
                (((integer)member_shift<struct chunk_hdr>(header_address, node) < (integer)hdr.node.next
                    && ha.start <= (integer)hdr.node.next
                    && (integer) hdr.node.next < end)
                || (integer)hdr.node.next == (integer)ha.head)
        );
        assert(
                check_node implies
                (
                ((integer)hdr.node.prev < (integer)member_shift<struct chunk_hdr>(header_address, node)
                    && ha.start <= (integer)hdr.node.prev
                    && (integer) hdr.node.prev < end)
                || (integer)hdr.node.prev == (integer)ha.head)
        );
...
predicate ({cn_chunk_hdr Hdr, struct list_head Node, integer va_size}) Cn_chunk_hdr_for_install(pointer header_address, pointer chunk, cn_hyp_allocator_core ha,  option_u64 alloc_size_opt, boolean valid_mapped_size)
{
        ...
        let next_chunk = (Cn_list_is_last(P.Node, ha.head) ? end : (integer)my_container_of_chunk_hdr(P.Node.next) );
        assert((integer)chunk < next_chunk);
        let va_size_2 =  next_chunk - (integer)chunk;
...
// function (bool) is_free_chunk(pointer p,integer size, datatype cn_chunk_hdrs hdrs)
// {
